...
set_defaults() {
    ...
    # 🟤 [错密黑名单] 防爆破减速带：错密阈值 / 拉黑时长 / 总开关（兼容无前缀旧别名）
    export RT_BLACKLIST_ENABLED="${RT_BLACKLIST_ENABLED:-${BLACKLIST_ENABLED:-true}}"
    export BLACKLIST_ENABLED="$RT_BLACKLIST_ENABLED"
    export RT_BLACKLIST_THRESHOLD="${RT_BLACKLIST_THRESHOLD:-${BLACKLIST_THRESHOLD:-5}}"
    export BLACKLIST_THRESHOLD="$RT_BLACKLIST_THRESHOLD"
    export RT_BLACKLIST_TTL_HOURS="${RT_BLACKLIST_TTL_HOURS:-${BLACKLIST_TTL_HOURS:-24}}"
    export BLACKLIST_TTL_HOURS="$RT_BLACKLIST_TTL_HOURS"
...
print_env_summary() {
    ...
    echo "   - 错密黑名单 [RT_BLACKLIST_ENABLED]: ${RT_BLACKLIST_ENABLED} (阈值 ${RT_BLACKLIST_THRESHOLD} 次 / 拉黑 ${RT_BLACKLIST_TTL_HOURS} 小时)"
