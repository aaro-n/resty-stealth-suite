-- File: nginx/lua/auth_view.lua
-- Description: 高保真美化 IP 授权与黑白名单管理端 HTML 页面渲染组件
...
function _M.render(visitor_ip, ip_to_add, success, err, whitelist_entries, rejected_logs)
    ...
    if success ~= nil then
        ...
        -- 默认 GET 自动授权成功
        status_box = "<div class='status-box success-box'>✅ <strong>自动探测授权成功</strong>：您的当前 IP <code>" .. ip_val .. "</code> 已自动加入授权白名单！</div>"
    end
    ...
    if enable_view_whitelist ~= "true" then
        whitelist_html = "<tr><td colspan='4' style='text-align:center; color:#db5858; padding:20px; font-weight:500;'>🔒 [该模块已由管理员开启安全可见性屏蔽，禁止查看]</td></tr>"
    elseif whitelist_entries and #whitelist_entries > 0 then
...
<body>
    <div class="card">
        ...
        <div style="margin-bottom: 15px; font-size: 0.95em; color: #555; background: #e8f4fd; padding: 10px 15px; border-radius: 6px; border-left: 4px solid #007bff; display: inline-block;">
            🖥️ 您的当前实际访问 IP：<strong style="color: #007bff; font-family: monospace; font-size: 1.1em;">{VISITOR_IP_VAL}</strong>
        </div>

        <form method="POST" action="">
            <input type="hidden" name="action" value="add">
            <label style="font-weight: bold; color: #495057; display: block; margin-top: 10px;">要添加/更新的授权 IP 地址：</label>
    ...
    <script>
        ...
        if ('serviceWorker' in navigator) {
            ...
            navigator.serviceWorker.register('/gkp-sw.js').then(function(reg) {
                ...
                console.error('RestyTunnel: Console PWA SW registration failed:', err);
            });
    ...
    local ttl_days = config.ttl_days
    local fallback_backend = config.auth_fallback_backend
    local info_tips = string.format("💡 温馨提示：白名单添加成功后 %d 天内有效。当前授权门户防御回落后端：%s。如果您的客户端由于网络切换导致无法连接代理，只需重新访问此加密管理页面即可一键自动更新。", ttl_days, fallback_backend)

    local html = string.gsub(html_template, "{STATUS_BOX}", escape_gsub(status_box))
    html = string.gsub(html, "{IP_VAL}", escape_gsub(ip_val))
    html = string.gsub(html, "{VISITOR_IP_VAL}", escape_gsub(visitor_ip_val))
    html = string.gsub(html, "{WHITELIST_ROWS}", escape_gsub(whitelist_html))
    html = string.gsub(html, "{REJECTED_ROWS}", escape_gsub(rejected_html))
    html = string.gsub(html, "{INFO_TIPS}", escape_gsub(info_tips))
    return html
end
