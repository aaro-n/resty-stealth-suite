...
为了实现抗主动探测的理论极限，我们重构了整个网关层，推出了**“基于 IP 白名单的定向 407 智能挑战机制”**：
* **对未授权外部 IP（非本人）**：整个 Nginx 与 Lua 网关层彻底剔除了 `407` 响应逻辑。任何时候、任何非加白设备在发起代理连接时，如果未在第一包请求中主动携带合法的 `Proxy-Authorization` 凭证，**服务器一律不返回 407 挑战，而是直接降维重定向（反代）至回落伪装后端（如 Bing 首页/云网盘）**。这使得外部扫描器探测时，永远无法摸出代理特征（扫描结果 100% 只是一个正常网站）。
* **对已加白 IP（您本人）**：Chrome / Edge / Firefox 搭配 SwitchyOmega / ZeroOmega 时，**冷启动第一次 `CONNECT` 不会携带代理凭据**（原因见下文，不是 TLS/HTTPS 协议硬性要求）。加白后网关才对其返回一次 `407 Proxy Authentication Required`，触发插件的 `onAuthRequired` 自动填密并重发，从而实现无缝直通。
...
1. **防御“未授权外部 IP 扫描”与“主动探测”的终极防火墙**
   ...
   * **白名单防御效果**：
     * **若开启白名单（`true`）**：除非攻击者能黑进您的控制台并通过您的手机 TOTP 认证，否则对于攻击者的 IP，网关层会冷酷地实行“静默洗白”。攻击者连代理密码校验的边缘都碰不到，首包直接被拦截并无声反代至伪装后端（如 Bing）。系统永远不向他们暴露任何 407 挑战或 401 报错，对外界呈现 100% 完美的普通网站行为。
     * **若不开启白名单（`false`）**：攻击者可以直接绕过 IP 过滤屏障。为了兼容浏览器，系统面对任何 IP 的无凭证 `CONNECT` 都会进行 407 挑战。这就给了全网扫描器一个巨大的指纹漏洞——扫描器能直接摸出 `407 Proxy Authentication Required` 挑战，从而将您的 IP 当场定性并封锁。
