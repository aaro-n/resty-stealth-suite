...
* **对已加白 IP（您本人）**：为了解决 Chrome/Edge/Firefox 等标准浏览器（以及 SwitchyOmega / ZeroOmega 插件）由于浏览器内核限制，在首包 `CONNECT` 中无法主动携带凭证的问题，我们在您完成 TOTP 授权并加白 IP 后，**允许服务器对其返回一次 407 Proxy Authentication Required 挑战**。这能瞬间触发浏览器的自动填充并再次发送，从而实现无缝、无感直通代理通道。
