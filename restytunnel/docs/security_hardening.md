...
### 🛡️ 3. 强制现代协议（HTTP/2 & HTTP/3）代理过滤机制 (新特性)
* **核心痛点：**
  HTTP/1.1 的 CONNECT 代理在公网上传输时指纹极其明显，很容易被深度包检测（DPI）识别。虽然我们在内存中进行了协议降维，如果允许客户端在公网上使用 HTTP/1.1 进行代理握手，依然存在暴露隐患。我们必须提供强力手段强制客户端使用高规格的 HTTP/2 或 HTTP/3 代理协议。
* **为什么这样做：**
  我们设计了 **`FORCE_MODERN_PROTO`** 环境变量，在内存对齐阶段提供三个级别的安全防护，在不改变 Nginx 原生代码的基础上提升传输安全性：
  * **模式一：`FORCE_MODERN_PROTO=proxy`（默认，最推荐的高隐蔽形态）：**
    * **逻辑：** 允许普通的访问、搜索引擎爬虫、或第三方主动探测机使用传统的 HTTP/1.1 协议访问你的域名（展示完美的静态站点首页 `200 OK`）。但是！一旦检测到请求带有正向代理特征（发起了 `CONNECT` 或是带有 `Proxy-Authorization`），则**强制审计其连接协议**。如果该代理握手运行在 HTTP/1.1 或更低版本中，Lua 瞬间拦截该请求并执行“降维装傻”重写，直接返回静态站点首页！
    * **结果：** 只有在完美的 **HTTP/2 (H2)** 或 **HTTP/3 (H3)** 加密多路复用通道里传过来的代理请求才能过关。恶意扫描器拿着传统的 1.1 CONNECT 工具来探测，其结果与访问普通个人网站完全一致，彻底断绝了通过 1.1 `CONNECT` 探测代理状态的可能。
  * **模式二：`FORCE_MODERN_PROTO=all`（极致绝对封锁形态）：**
    * **逻辑：** 整个 443 端口在最外层彻底切断对 HTTP/1.1 或更低版本的一切响应！不管对方是来看网站、盲扫、还是其他请求，只要 TLS ALPN 握手最终协商为了 HTTP/1.1，Nginx 立刻通过 `ngx.exit(ngx.HTTP_CLOSE)` 强制阻断并掐掉 TCP 链接。
    * **结果：** 整台服务器 443 端口全盘纯净化为只吃 H2 / H3 现代流量的钢板一块，杜绝了一切老旧低级工具和扫描器的握手和交互机会。
  * **模式三：`FORCE_MODERN_PROTO=none`（兼容调试形态）：**
    * **逻辑：** 不作任何协议版本限制。支持在一些特殊的不支持 ALPN / HTTP2 代理的旧客户端下进行临时兼容测试。
